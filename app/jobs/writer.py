...
def _writer_quiescent_status(
    previous_status: ProposalStatus | str | None,
) -> ProposalStatus:
    """Return the honest non-running status after a writer startup failure."""
    if previous_status == ProposalStatus.AWAITING_DRAFT:
        return ProposalStatus.AWAITING_DRAFT
    if previous_status in (
        ProposalStatus.DRAFT_READY,
        ProposalStatus.REVIEWING,
    ):
        return ProposalStatus.DRAFT_READY
    return ProposalStatus.AWAITING_DRAFT


def _restore_writer_quiescent_status(
    proposal_id: int,
    previous_status: ProposalStatus | str | None,
) -> None:
    """Clear a stale DRAFT_IN_PROGRESS marker after startup failure.

    Only change the row if it still carries this writer run's busy marker;
    that avoids overwriting a newer status written by another workflow.
    """
    target_status = _writer_quiescent_status(previous_status)
    try:
        with session_scope() as db:
            proposal = db.get(Proposal, proposal_id)
            if (
                proposal is not None
                and proposal.status == ProposalStatus.DRAFT_IN_PROGRESS
            ):
                proposal.status = target_status
    except Exception:
        log.exception(
            "writer team could not restore proposal %d to status %s",
            proposal_id,
            target_status,
        )
...
def run_writer_team(proposal_id: int) -> None:
    ...
    """
    require_proposal_mutable(proposal_id, operation="run writer team")
    log.info("writer team starting for proposal %d", proposal_id)
    previous_status: ProposalStatus | str | None = None
    ...
    try:
        ...
        if not snap["sections"]:
            _restore_writer_quiescent_status(proposal_id, previous_status)
            _set_stage(
                proposal_id,
                "No outline to draft from — generate the outline first.",
                status="failed",
            )
            return
        ...
        # redraft.
        # Claim each eligible section before any provider work. The ownership
        # operation is atomic, unlike a get-active-then-start check, so a
        # manual regenerate, auto-review worker, or polish pass cannot race the
        # batch between its snapshot and persistence.
        ...
        for section in snap["sections"]:
            ...
            if not add_active_section(proposal_id, section["pk"]):
                ...
                log.info(
                    ...
                    "worker owns it right now; not racing.",
                    section["section_id"], section["pk"],
                )
                continue
            claimed_pks.add(section["pk"])
        ...
        with ThreadPoolExecutor(
            ...
            for future in as_completed(future_to_section):
                ...
                try:
                    ...
                    claimed_pks.discard(section["pk"])
        ...
        if n_skipped_cost:
            msg += (
                f" {n_skipped_cost} cost section(s) were left unchanged; "
                f"they are handled separately by the Cost Volume Writer."
            )
        if n_skipped_excluded:
            msg += (
                f" {n_skipped_excluded} section(s) excluded from draft "
                f"(user-flagged)."
            )
        _set_stage(
            proposal_id,
        ...
        _set_stage(
            proposal_id,
            "Writer Team failed — check logs.",
            status="failed",
        )
    finally:
        # Covers startup/submission failures before the per-future cleanup.
        # Only this orchestrator thread can release the leases it acquired.
        for section_pk in tuple(claimed_pks):
...
def run_writer_for_section(
    ...
    Returns True only after the replacement draft has been persisted and
    post-processing completed. The prior draft remains intact on failure.
    """
    require_proposal_mutable(
        proposal_id, operation="regenerate proposal section",
    )
    log.info(
    ...
    acquired = add_active_section(proposal_id, proposal_section_pk)
    if not acquired:
        _set_stage(
            proposal_id,
            f"Section pk={proposal_section_pk} is already being changed by "
            "another writer, reviewer, or polish worker. Regeneration was "
            "skipped; try again after that work completes.",
            status="failed",
        )
        return False
    try:
        ...
        if section is None:
            _set_stage(
                proposal_id,
                f"Section pk={proposal_section_pk} not found.",
                status="failed",
            )
            return False
        ...
        if section.get("requires_cost_analysis"):
            _set_stage(
                proposal_id,
                f"Section {section['section_id']} is a cost section handled "
                f"by the Cost Volume Writer; regular Writer Team regeneration "
                f"was skipped.",
            )
            return False
        ...
        # Snapshot the user's resolved [NEEDS_HUMAN] answers. These are
        # passed to the Writer Team prompt so it bakes the values into the
        # new prose, and re-applied as a safety net after persistence in
        # case the model still re-emits matching markers. Keep the existing
        # draft in place until a complete replacement is ready.
        ...
        _set_stage(
            proposal_id,
            "Section regenerate failed — check logs.",
            status="failed",
        )
        return False
    finally:
